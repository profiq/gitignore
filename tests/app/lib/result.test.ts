...
describe("removeDuplicates()", () => {
  test("remove (comment) duplicate lines", async () => {
    const input = fs.readFileSync("./tests/assets/C_C++.gitignore", "utf8");
    const output = fs.readFileSync("./tests/assets/remDupl_output.gitignore", "utf8");
    expect(await removeDuplicates(input)).toBe(output);
  });

  test("remove (comment) duplicate lines: withou t duplicates", async () => {
    const input = fs.readFileSync("./tests/assets/templates/C.gitignore", "utf8");
    expect(await removeDuplicates(input)).toBe(input);
  });
});
...
describe("createNeededFilesDict", () => {
...
describe("getResult", () => {
  test("getResult: c, c++, remdupl=false", async () => {
    ...
    const expected = fs.readFileSync("./tests/assets/C_C++.gitignore", "utf8");
  ...
  test("getResult: ReactNative, Android", async () => {
    ...
    const expected = fs.readFileSync(
      "./tests/assets/ReactNative_Android.gitignore",
      "utf8",
    );
